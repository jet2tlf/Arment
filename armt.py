...
class VarAssignNode:
    def __init__(self, var_name_tok, value_node, extra_names=None):
        ...
        self.pos_end = self.extra_names[len(self.extra_names) - 1].pos_end if len(
            self.extra_names) > 0 else self.var_name_tok.pos_end
...
class Parser:
    ...
    def class_node(self):
        ...
        if not self.current_tok.matches(TT_KEYWORD, 'class'):
            return res.failure(InvalidSyntaxError(self.current_tok.pos_start, self.current_tok.pos_end, f"Expected 'class'"))
...
class RTResult:
    ...
    def should_return(self):
        return self.error or self.func_return_value or self.loop_should_continue or self.loop_should_break or self.should_exit
...
class Class(Value):
    ...
    def execute(self, args):
        ...
        if method is None or not isinstance(method, Function):
            return res.failure(RTError(self.pos_start, self.pos_end, f"Function '{self.name}' not defined", self.context))
...
class Interpreter:
    ...
    def visit_VarAccessNode(self, node, context):
        ...
        if node.child:
            if not isinstance(value, Instance) and not isinstance(value, Class):
                return res.failure(RTError(node.pos_start, node.pos_end, f"Value must be instance of class or class", context))
    ...
    def visit_ClassNode(self, node, context):
        ...
        cls_ = Class(node.class_name_tok.value, ctx.symbol_table).set_context(context).set_pos(node.pos_start,
                                                                                               node.pos_end)
